...
class Query(DBConnection):
    ...
    def insert_new_issued_number(self, id_of_number, number_itself, db, is_uber=None, is_yango=None):

        if is_uber:

            db.execute(f"INSERT INTO uber_number (portal_id, mobile_no) VALUES ({id_of_number}, "
                        f"{number_itself});")

        if is_yango:

            db.execute(f"INSERT INTO yango_number (portal_id, mobile_no) VALUES ({id_of_number}, "
                        f"{number_itself});")
    ...
    @complete_query(is_commit=True)
    def update_trip_info(self, portal_id, db, is_uber=None, is_yango=None): # increments by 1
        """
        ...
        if is_uber:
            db.execute(f"UPDATE uber_number SET trips_completed = trips_completed + 1 WHERE portal_id = {portal_id};")

        if is_yango:
            db.execute(f"UPDATE yango_number SET trips_completed = trips_completed + 1 WHERE portal_id = {portal_id};")



    @complete_query(is_commit=True)
    def insert_confirmation_code(self, portal_id, confirmation_code, db, is_uber=None, is_yango=None):

        # response_id = self.get_response_id(id_of_number)

        if is_uber:
            db.execute(f"UPDATE uber_number SET confirmation = {confirmation_code} WHERE portal_id = {portal_id};")

        if is_yango:
            db.execute(f"UPDATE yango_number SET confirmation = {confirmation_code} WHERE portal_id = {portal_id};")
    ...
    def get_mobile_no(self, id_of_number, db, is_uber=None, is_yango=None):
        ...
        if is_uber:
            db.execute(f"SELECT mobile_no FROM uber_number WHERE portal_id = {id_of_number};")

        if is_yango:
            db.execute(f"SELECT mobile_no FROM yango_number WHERE portal_id = {id_of_number};")
    ...
    def insert_driver_data(self, name, phone, details, db, is_uber=None, is_yango=None):
        ...
        if is_uber:
            db.execute(f"INSERT INTO uber_driver_data (name, number, details, date) VALUES (%s, %s, %s, %s::timestamp);", (name, phone, details, datetime.now().strftime("%Y-%m-%d %H:%M:%S"),))
        if is_yango:
            db.execute(f"INSERT INTO yango_driver_data (name, number, details, date) VALUES (%s, %s, %s, %s::timestamp);", (name, phone, details, datetime.now().strftime("%Y-%m-%d %H:%M:%S"),))



if __name__ == "__main__":

    query = Query(Utils.get_json_to_dict(os.path.dirname(os.getcwd()) + '/config/database_config.json')['DB_conn_str'])


    query.insert_driver_data('arif', '519234212', 'Gray Honda Insight 77XZ712', is_yango=True)
