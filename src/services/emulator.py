...
class Emulator(Appium):
    ...
    def list_active_adb_emulators(self):
        return subprocess.check_output(f"adb -s {self.device_name} devices", shell=True).decode()

    def stop_adb_server(self):
        os.system(f'adb -s {self.device_name} kill-server')

    def start_emulator(self):
        os.environ["ANDROID_EMULATOR_HOME"] = os.environ["ANDROID_SDK_ROOT"] + "/emulator/emulator"
        ...
        subprocess.Popen(
            ...
            stdout=subprocess.PIPE, env=os.environ.copy(), shell=True)
    ...
    def kill_emulator(self):
        subprocess.Popen(f"adb -s {self.device_name} devices | grep emulator | cut -f1 | while read line; do adb -s $line emu kill; done",
                         shell=True)
...
if __name__ == "__main__":
    xxx = Emulator(Utils.get_json_to_dict(os.path.dirname(os.getcwd()) + '/config/emulator_config.json'))
